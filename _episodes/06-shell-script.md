...
Let's start by going back to `protein/` and creating a new file, `middle.sh` which will
...
Check that the directory `protein` now contains a file called `middle.sh`.
...
>
> We can use the command `cut -d , -f 2 counts.txt | sort | uniq` to produce
> the unique species in `counts.txt`.
...
If we want to make our analytics must be reproducible,
...
>
> In the `protein` directory, imagine you have a shell script called `script.sh` containing the
> following commands:
>
> ~~~
> head -n $2 $1
...
> {: .language-bash}
>
> While you are in the `protein` directory, you type the following command:
>
> ~~~
...
>    in the `protein` directory
> 2. The first and the last line of each file ending in `.pdb` in the `protein` directory
> 3. The first and the last line of each file in the `protein` directory
...
>
> For this question, consider the `shell-lesson-data/ex-data/protein` directory once again.
...
> in thr `ex-files/scripts` directory:
>
> ~~~
> # Calculate stats for data files.
> for datafile in "$@"
...
> {: .language-bash}
>
> When you run it from the `ex-files` directory:
>
> ~~~
